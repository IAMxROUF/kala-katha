...
export default async function handler(req, res) {
  ...
  // Prefer OpenAI; fall back to Gemini for backward compat
  ...
  if (process.env.GEMINI_API_KEY) {
    try {
      const result = await callGemini({ rawStory, productName, craft, region })
      return res.status(200).json(result)
    } catch (err) {
  ...
  return res.status(500).json({
    error: 'Neither OPENAI_API_KEY nor GEMINI_API_KEY is configured on the server.',
  })
}
...
async function callGemini({ rawStory, productName, craft, region }) {
...
Focus on the PRODUCT, not the artisan's personal story.
Return ONLY a valid JSON object.`
...
Artisan's notes:
"""
${rawStory || 'A beautiful handcrafted piece.'}
"""

Return a JSON object with: description (about the product), materials, technique, time.`
